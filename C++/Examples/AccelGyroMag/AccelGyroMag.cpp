...
unsigned long int auxCount=0,count=0;
...
//-------------------------------------------------------------------------
    ...
    while(count<5000) {
		...
		timersub(&t1, &t0, &dt);
		imuData.push_back(dt.tv_usec-20000);
        count++;
        usleep(10000);

    }

    FILE *fBaro = fopen("mpu.txt", "w");
    	fprintf(fBaro, "count;dtMpu\n");
    	fclose(fBaro);
    	for (std::vector<int>::iterator it = imuData.begin() ; it != imuData.end(); ++it){
    		auxCount++;
    		FILE *fBaro = fopen("mpu.txt", "a");
    		fprintf(fBaro, "%d;%d\n",auxCount,*it);
    		fclose(fBaro);
    	}
}
